...
class Auto_Upgade_button():
    def __init__(self, name, cost, cost_pow, upgade_pow):  # 이름, 비용, 비용 증가량, 업그레이드 수치
        ...
        self.name = StringVar()
        self.name.set("loding")
        self.button = Button(window, textvariable=self.name, command=self.Buy_Auto_Gold).pack(
            side=TOP, anchor=S)
        self.UI_Update()
...
class Click_Upgade_button():
    def __init__(self, name, cost, cost_pow, upgade_pow):  # 이름, 비용, 비용 증가량, 업그레이드 수치
        ...
        self.upgade_pow = upgade_pow
        self.name = name + "\n 비용 : " + \
            str(cost) + "gold/click 증가량: " + str(upgade_pow)
        self.button = Button(
            window, text=self.name, command=self.Buy_Click_Upgade).pack(side=TOP, anchor=S)
